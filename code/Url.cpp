...
Url::Url(const std::string& url) : myData(url) {
  const int result =
      ::http_parser_parse_url(myData.data(), myData.size(), 0, &myFields);
  if (result != 0) {
    // TODO: handle failure.
  }
}

Url::Url(const char* data, std::size_t size) : myData(data, size) {
  const int result =
      ::http_parser_parse_url(myData.data(), myData.size(), 0, &myFields);
  if (result != 0) {
    // TODO: handle failure.
